...
foreach ($id in $seoTasks) {
    try {
        ...
        $name = $task.name.Substring(0, [Math]::Min(60, $task.name.Length))
        Write-Host "  Task $id - Status: $($task.status.status) - $name" -ForegroundColor Yellow
    } catch {
        Write-Host "  Task $id - ERROR" -ForegroundColor Red
    }
}
