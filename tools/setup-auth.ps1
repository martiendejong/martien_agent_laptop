...
<#
.SYNOPSIS
    Setup Hazina.Auth authentication system for any ASP.NET project

.DESCRIPTION
    Automates adding authentication (Identity + JWT + OAuth) to any project:
    - Adds NuGet packages
    - Creates AuthController
    - Updates Program.cs
    - Adds appsettings.json configuration
    - Optionally copies React frontend components
    - Generates EF migration
...
Write-Host "=== Hazina.Auth Setup ===" -ForegroundColor Cyan
...
Write-Host "[1/7] Adding NuGet packages..." -ForegroundColor Yellow
cd $ProjectPath
...
$templatePath = "C:\Projects\hazina\templates\backend\AuthController.cs.template"
...
if ($FrontendPath) {
    ...
    $frontendTemplatesDir = "C:\Projects\hazina\templates\frontend\auth"
