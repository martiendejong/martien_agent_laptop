...
param(
    [string]$OutputPath = "C:\temp\team-yesterday.html"
)

# Fixed to yesterday only
$Days = 1
$Format = "html"
$AllRepos = $false
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 50%;">ClickUp Tasks</th>
                        <th style="width: 30%;">Branches & Commits</th>
                    </tr>
                </thead>
                <tbody>
"@
        ...
        $yesterdayKey = $allDays[0]  # Since Days=1, this is yesterday

        foreach ($person in $reportData) {
            $dayData = $person.days[$yesterdayKey]

            if (-not $dayData.hasActivity) {
                continue  # Skip people with no activity
            }

            $html += "<tr>"

            # Column 1: Name
            $html += "<td class='user-name'>👤 $($person.user)</td>"
            ...
            if ($dayData.commits -gt 0) {
                $html += "<div style='display: inline-block; background: #667eea; color: white; padding: 4px 12px; border-radius: 20px; font-weight: bold; margin-bottom: 10px;'>$($dayData.commits) commits</div><br>"
            }

            # Get branches from userActivity data
            $username = $person.user
            if ($userActivity.ContainsKey($username) -and $userActivity[$username].ContainsKey($yesterdayKey)) {
                $branches = $userActivity[$username][$yesterdayKey].branches
                if ($branches.Count -gt 0) {
                    $html += "<ul style='list-style: none; padding: 0; font-family: monospace; font-size: 13px;'>"
                    foreach ($branch in $branches) {
                        $html += "<li style='margin-bottom: 6px; padding: 6px 10px; background: #f8f9fa; border-left: 3px solid #667eea; border-radius: 3px;'>$($branch.name)</li>"
                    }
                    $html += "</ul>"
                }
            }

            if ($dayData.commits -eq 0) {
