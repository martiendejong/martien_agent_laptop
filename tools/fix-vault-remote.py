...
def main():
    ...
    for key_path in key_paths:
        try:
            print(f"Attempting connection with key: {key_path}")
            client.connect(host, username=username, key_filename=key_path, timeout=10)
            connected = True
            print("✓ Connected successfully with SSH key\n")
            break
        except FileNotFoundError:
            print(f"  Key not found: {key_path}")
        except paramiko.AuthenticationException:
            print(f"  Authentication failed with key: {key_path}")
        except Exception as e:
            print(f"  Error: {e}")

    # Try password authentication if keys failed
    if not connected:
        print("\nSSH key authentication failed. Trying password...")
        try:
            password = input("Enter password for administrator@85.215.217.154: ")
    ...
    try:
        ...
        if output.lower() != 'true':
            print(f"❌ Site path not found: {site_path}")
...
"""
        ...
        if 'Config updated' in output:
            print(f"✓ Connection string updated to: Data Source={db_path}\n")
        else:
            print(f"⚠ {output}\n")
...
"""
        ...
        if output:
            ...
            print("  ⚠ Could not retrieve certificate info\n")
        ...
        execute_command(client,
            ...
            f"Restart {app_pool}")
        print("✓ Application pool restarted\n")
        ...
        if output.lower() == 'true':
            ...
            print("⚠ Database file not yet created (will be created on first request)\n")
